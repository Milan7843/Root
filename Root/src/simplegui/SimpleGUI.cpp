...
unsigned int SimpleGUI::quadVAO{ 0 };
unsigned int SimpleGUI::quadVBO{ 0 };
unsigned int SimpleGUI::quadEBO{ 0 };

SimpleGUI::SimpleGUI(unsigned int windowWidth, unsigned int windowHeight)
{
...
SimpleGUI::~SimpleGUI()
{
    ...
    for (SimpleGUIComponent::Item* p : renderQueue)
    {
        delete p;
    }

	std::cout << "SimpleGUI instance destroyed." << std::endl;
}

void SimpleGUI::createShaderPrograms()
{
    /* GUI SHADER */
...
void SimpleGUI::render(unsigned int width, unsigned int height)
{
    // Draw GUI on top of everything
    glDisable(GL_DEPTH_TEST);
    for (SimpleGUIComponent::Item* item : renderQueue)
    {
        item->render(guiShader, textShader, width, height);
    }
}

void SimpleGUI::addItemToRenderQueue(SimpleGUIComponent::Item* item)
{
